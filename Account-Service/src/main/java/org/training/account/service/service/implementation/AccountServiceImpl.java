...
public class AccountServiceImpl implements AccountService {
    ...
    private final SequenceService sequenceService = null;
    ...
    public Response createAccount(AccountDto accountDto) {
        ...
        accountRepository.findAccountByUserIdAndAccountType(accountDto.getUserId(), AccountType.valueOf(accountDto.getAccountType()))
                .ifPresent(account -> {
                    System.err.println("Account already exists on the server");
    ...
    public Response updateStatus(String accountNumber, AccountStatusUpdate accountUpdate) {
        return accountRepository.findAccountByAccountNumber(accountNumber)
                .<Response>map(account -> {  // Explicit type specification
    ...
    public Response updateAccount(String accountNumber, AccountDto accountDto) {
        return accountRepository.findAccountByAccountNumber(accountNumber)  // Use the method parameter instead
                .<Response>map(account -> {  // Explicitly specify the return type
                    BeanUtils.copyProperties(accountDto, account);
                    accountRepository.save(account);
                    return Response.builder()
                            .responseCode(success)  // Make sure 'success' is defined
                            .responseMessage("Account updated successfully")
                            .build();
                })
    ...
    public Response closeAccount(String accountNumber) {
        return accountRepository.findAccountByAccountNumber(accountNumber)
                .<Response>map(account -> {  // Explicitly specify the Response type
                    BigDecimal balance = BigDecimal.valueOf(Double.parseDouble(getBalance(accountNumber)));
                    if (balance.compareTo(BigDecimal.ZERO) != 0) {
                        throw new AccountClosingException("Balance should be zero");
                    }
                    account.setAccountStatus(AccountStatus.CLOSED);
                    accountRepository.save(account);  // Don't forget to save the changes
                    return Response.builder()
                            .responseMessage("Account closed successfully")
                            .responseCode(success)  // Make sure 'success' is properly defined
                            .build();
                })
