...
public class FundTransferServiceImpl implements FundTransferService {
    ...
    private final FundTransferRepository fundTransferRepository = null;
    ...
    public FundTransferResponse fundTransfer(FundTransferRequest fundTransferRequest) {
        ...
        if(Objects.isNull(response.getBody())){
            System.err.println("requested account "+fundTransferRequest.getFromAccount()+" is not found on the server");
        ...
        if (!fromAccount.getAccountStatus().equals("ACTIVE")) {
        	System.err.println("account status is pending or inactive, please update the account status");
        ...
        if (fromAccount.getAvailableBalance().compareTo(fundTransferRequest.getAmount()) < 0) {
        	System.err.println("required amount to transfer is not available");
        ...
        if(Objects.isNull(response.getBody())) {
        	System.err.println("requested account "+fundTransferRequest.getToAccount()+" is not found on the server");
            throw new ResourceNotFound("requested account not found on the server", GlobalErrorCode.NOT_FOUND);
        }
